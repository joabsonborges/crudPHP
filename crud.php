...
//*****************************************************************************
// Metodo que carrega lista de pessoas cadastrados
//
function carregarLista() {
    ...
    //preara e executa consulta de lista de pessoas
...
//*****************************************************************************
// Metodo que carrega dados do pessoa selecionado para alteracao
//
function carregarCliente() {
    ...
    if ( isset( $_POST["id"] ) && is_numeric( $_POST["id"] ) ) {
        ...
        $stmt = $mysqli->prepare("SELECT * FROM pessoa WHERE id=?");
...
//*****************************************************************************
// Metodo que salva ou atualiza form de cadastro do pessoa
//
function salvarForm() {
    //var_dump($_POST);
    if ( ! isset( $_POST ) || empty( $_POST ) ) {
    ...
    $sql = null;
    if ( $id > 1 ) {
        $sql = "UPDATE pessoa SET nome=?, email=?, localidade=?, telefone=?, foto=? WHERE id=".$id;
    } else {
        $sql = "INSERT INTO pessoa (nome, email, localidade, telefone, foto) VALUES (?, ?, ?, ?)";
    }
    //prepara e executa sql para insert dos dados
    $stmt = $mysqli->prepare( $sql );
    $stmt->bind_param('ssis', $nome, $email, $localidade, $telefone, $nome_imagem); 
    $stmt->execute();
...
//*****************************************************************************
// Metodo que exclui registro do pessoa
//
function excluirForm() {
    ...
    if ( isset( $_POST["id"] ) && is_numeric( $_POST["id"] ) ) {
        ...
        $stmt = $mysqli->prepare("DELETE FROM Pessoa WHERE id=?");
        $stmt->bind_param('i', $id); 
        $stmt->execute();
...
function validarForm( $id, $nome, $email, $telefone, $foto ) {
    ...
    if ( $email == null || trim( $email ) == "" ) {
     ...
     //validar campo localidade
    if ( $localidade == null || trim( $localidade ) == "" ) {
        return "Campo Localidade deve ser preenchido.";
    }
